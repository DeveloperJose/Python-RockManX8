...
//void D3D11CustomContext::Notify_Present(IDXGISwapChain4* p_swap_chain, UINT sync_interval, UINT present_flags, const DXGI_PRESENT_PARAMETERS* p_present_params)
//{
...
D3D11CustomContext::D3D11CustomContext(ID3D11DeviceContext* devCon, Editor* Parent)
{
	CommonInitialise();
	m_devContext = devCon;
	m_pGLOM = Parent;
...
D3D11CustomContext::D3D11CustomContext(ID3D11DeviceContext* dev, D3D11CustomDevice* cdev, Editor* Parent)
{
	CommonInitialise();
	m_devContext = dev;
	m_pFalseDevice = cdev;
