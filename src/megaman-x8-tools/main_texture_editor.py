...
def __main__():
    ...
    parser.add_argument(
        "option", type=str, help="Task for the editor. Either 'export' or 'import'"
    )
    parser.add_argument(
        "--import_folder",
        type=str,
        help="When importing, what folder you are importing",
    )
    ...
    if not opk_path.exists():
        print(
            ">> opk folder not found, please place this script in the root directory of the game files"
        )
        return
    ...
    if args.option.lower() == "export":
        ...
        for idx, wpg_backup_path in enumerate(all_wpg_files):
            printProgressBar(
                idx,
                len(all_wpg_files),
                '>> Exporting all textures (*.wpg) to "exported_textures" directory',
            )
            wpg = WPGFile(wpg_backup_path)
            texture_folder = (
                export_dir
                / wpg_backup_path.relative_to(backup_opk).parent
                / wpg_backup_path.stem
            )
        ...
        wpg_backup_path = (
            backup_opk / import_dir.relative_to(export_dir).parent / import_dir.stem
        ).with_suffix(".wpg")
        if not wpg_backup_path.exists():
            print(
                ">> Could not find the matching backup WPG file, did you rename any folders or files in the 'backup_opk' directory?"
            )
            return

        wpg_real_path = (
            opk_path / import_dir.relative_to(export_dir).parent / import_dir.stem
        ).with_suffix(".wpg")
        if not wpg_backup_path.exists():
            print(
                ">> Could not find the matching real WPG file, did you rename any folders or files in the 'opk' directory?"
            )
            return
