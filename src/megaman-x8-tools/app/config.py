...
def load_config_or_default():
    if config_path.exists():
        ...
        is_valid_collection = (
            install_path / "nativeDX10" / "X8" / "romPC" / "data" / "mes"
        ).exists()
...
def __prompt_language__():
    prompt, language_selected = QInputDialog.getItem(
        None,
        "Language Select",
        "Please select the language to edit".ljust(50),
        languages,
        0,
        False,
    )
...
def __prompt_installation__():
    while True:
        install_fname = QFileDialog.getExistingDirectory(
            None,
            caption="Please select your X8 or Legacy Collection installation folder",
            directory="",
        )
        if install_fname == "":
            return False
