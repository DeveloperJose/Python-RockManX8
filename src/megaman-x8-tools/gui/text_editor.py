...
class TextEditorWindow(QMainWindow):
    ...
    def __init_editor_group__(self):
        ...
        self.ui.spinCurrentText.valueChanged.connect(
            lambda new_value: self.ui_update_editor()
        )
    ...
    def evt_clicked_save(self, checked):
        ...
        if self.mcb.has_extras():
            ...
            voice_idx = (
                0xFFFF if voice_name == "None" else self.ui.comboVoice.currentIndex()
            )
            ...
            char_idx = (
                0xFFFF if char_name == "None" else self.ui.comboCharacter.currentIndex()
            )
            ...
            mugshot_pos_idx = MCBExtra.MugshotPosition[
                self.ui.comboMugshotPos.currentText()
            ]
            text_pos_idx = MCBExtra.TextPosition[
                self.ui.comboTextPos.currentText()
            ].value
            ...
            close_top_idx = (
                1 if text_pos_idx == MCBExtra.TextPosition.Top else close_top_idx
            )
    ...
    def ui_update_mugshot(self, mug_idx):
        ...
        self.ui.textMugshotDesc.setText(
            MCBExtra.get_mugshot_description(char_idx, mug_idx)
        )
    ...
    def ui_update_editor(self):
        ...
        self.ui.lblTotalTexts.setText(
            f'<span style=" color:#aa0000;">{total_texts}</span>'
        )
    ...
    def ui_update_extra_data(self):
        ...
        voice_idx = (
            extra.voice
            if extra.voice != 0xFFFF
            else self.ui.comboVoice.findText("None")
        )
        bgm_idx = (
            extra.bgm if extra.bgm != 0xFFFF else self.ui.comboBGM.findText("None")
        )
        char_idx = (
            extra.char
            if extra.char != 0xFFFF
            else self.ui.comboCharacter.findText("None")
        )
